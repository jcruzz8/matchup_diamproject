...
const MessagesPage = () => {
                    ...
                    if (otherUserId) {
                        ...
                        if (otherUser) {
                            updateInbox(otherUserId, false, {
                                ...
                                members: [] // Jogadores sozinhos não têm lista de membros
                            }, msg);
                        }
                    }
                }
    ...
    const handleSendMessage = async (e) => {
        ...
        if (activeChat.isTeam) {
            payload.team = activeChat.id;
        } else {
            payload.receiver = activeChat.id; // Garante que envia o ID correto do recetor
        }
    ...
    {/* RESOLUÇÃO DO PROBLEMA 3: LÓGICA DE PESQUISA CORRIGIDA */}
    const currentUser = allPlayers.find(p => p.id === userId);
    
    // Agora o React procura TODOS os jogadores (caso não tenhamos os following guardados no serializador)
    // ou apenas na lista de 'colegas' garantida.
    const myFriendsIds = currentUser?.colegas || []; 
    ...
    return (
        <div className="bg-light vh-100 d-flex flex-column" style={{ paddingTop: '56px', paddingBottom: '65px' }}>
                                    ...
                                    >
                                        <div className="d-flex align-items-center overflow-hidden">
                                            <div className={`rounded-circle overflow-hidden me-3 border flex-shrink-0 position-relative ${chat.isTeam ? 'bg-dark' : 'bg-secondary'}`} style={{ width: '50px', height: '50px' }}>
                                                {chat.avatar ? (
                                                    <img src={chat.isTeam ? `http://localhost:8000${chat.avatar}` : (chat.avatar.startsWith('/') ? `http://localhost:8000${chat.avatar}` : chat.avatar)} alt="user" className="w-100 h-100 object-fit-cover"/>
                                                ) : (
                                ...
                                <div className="bg-white border-bottom shadow-sm px-3 d-flex align-items-center z-1 w-100" style={{ height: '70px' }}>
                                    ...
                                    <div className="d-flex align-items-center cursor-pointer" onClick={() => activeChat.isTeam ? setShowMembersModal(true) : navigate(`/perfil/${activeChat.id}`)}>
                                        <div className={`rounded-circle overflow-hidden me-3 shadow-sm border ${activeChat.isTeam ? 'bg-dark' : 'bg-secondary'}`} style={{ width: '45px', height: '45px' }}>
                                            {activeChat.avatar ? (
                                                <img src={activeChat.isTeam ? `http://localhost:8000${activeChat.avatar}` : (activeChat.avatar.startsWith('/') ? `http://localhost:8000${activeChat.avatar}` : activeChat.avatar)} alt="user" className="w-100 h-100 object-fit-cover"/>
                                            ) : (
            ...
            {/* RESOLUÇÃO DO PROBLEMA 1: MODAL DOS MEMBROS DA EQUIPA */}
            <Modal isOpen={showMembersModal} toggle={() => setShowMembersModal(false)} centered className="rounded-4">
                ...
                <ModalBody className="pt-3">
                    <ListGroup flush>
                        {activeChat?.isTeam && activeChat.members?.length > 0 ? (
                            ...
                            <div className="text-center py-4 text-muted small">Não foi possível carregar os membros.</div>
                        )}
                    </ListGroup>
                </ModalBody>
            </Modal>
                ...
                <ModalBody className="pt-3">
                    ...
                    {searchFriend.trim() === '' || filteredFriends.length === 0 ? (
                        <div className="text-center py-4">
                            ...
                            <h6 className="fw-bold text-dark mb-1">Não foram encontrados amigos.</h6>
                            <p className="text-muted small mb-4">A pesquisa é feita entre os teus colegas de equipa e os teus amigos mútuos.</p>
                            ...
                            {filteredFriends.map(friend => (
                                ...
                                >
                                    <div className="rounded-circle overflow-hidden bg-secondary me-3 d-flex justify-content-center align-items-center text-white fw-bold shadow-sm" style={{ width: '45px', height: '45px' }}>
                                            {friend.photo ? (
                                            <img src={friend.photo.startsWith('/') ? `http://localhost:8000${friend.photo}` : friend.photo} alt="user" className="w-100 h-100 object-fit-cover"/>
                                        ) : (
